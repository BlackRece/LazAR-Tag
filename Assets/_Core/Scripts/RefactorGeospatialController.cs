namespace BlackRece.LaSARTag.Geospatial
{
    ...
    public class RefactorGeospatialController : MonoBehaviour
    {
        ...
        public void OnEnable()
        {
            ...
            _shouldResolvingHistory = _historyCollection.Count > 0;
        ...
        public void Update()
        {
            ...
            var pose = _IsEarthTracking ? EarthManager.CameraGeospatialPose : new GeospatialPose();
            if (!isSessionReady || !_IsSessionTracking ||
        ...
        private ARGeospatialAnchor PlaceGeospatialAnchor(AnchorParams anchorData, bool terrain = false)
        {
            Quaternion eunRotation = anchorData.EunRotation;
            ...
            ARGeospatialAnchor anchor = terrain ?
                ...
                AnchorManager.AddAnchor(
                    anchorData.Latitude, anchorData.Longitude, anchorData.Altitude, eunRotation);
            if (anchor != null)
            {
        ...
        private void ResolveHistory()
        {
            ...
            foreach (var history in _historyCollection)
        ...
        private void LoadGeospatialAnchorHistory()
        {
            if(_historyCollection == null)
                _historyCollection = new List<AnchorParams>();
        ...
        private void SaveGeospatialAnchorHistory()
        {
            ...
            _historyCollection.Sort((left, right) =>
                right.CreatedTime.CompareTo(left.CreatedTime));

            // Remove the earliest data if the capacity exceeds storage limit.
            if (_historyCollection.Count > _storageLimit)
            {
                _historyCollection.RemoveRange(
                    _storageLimit, _historyCollection.Count - _storageLimit);
            }
